...
A common pattern to do while using GQty is to just access manually some specified fields, so GQty can pick up your requirements faster.
...
```ts
import { getFields, query, resolved } from 'gqty';
...
```ts
import { getArrayFields, query, resolved } from 'gqty';
...
by logic you can be completely sure that in runtime you are not dealing with actual `undefined`s, GQty also exports no-op functions
