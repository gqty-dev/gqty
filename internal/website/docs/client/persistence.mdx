...
- React
  ...
  - [useQuery](/docs/react/fetching-data#usequery) and [graphql HOC](/docs/react/fetching-data#graphql-hoc) `staleWhileRevalidate`,
  - [useTransactionQuery](/docs/react/fetching-data#usetransactionquery) and [useLazyQuery](/docs/react/fetching-data#uselazyquery) [`cache-and-network` fetchPolicy](/docs/react/fetching-data#fetch-policy),

  And setting them by default in [your defaults](/docs/react/config#defaults).

- Core
  - [resolved](/docs/client/fetching-data#resolved)'s [onCacheData](/docs/client/api/interfaces/resolveoptions#oncachedata) function.
