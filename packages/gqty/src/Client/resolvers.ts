...
export function createResolvers(
  ...
  const resolveQueryPromisesMap = new Map<string, Promise<any>>();
  ...
  async function buildAndFetchSelections<TData = unknown>(
    ...
    let promise: Promise<TData | undefined> | undefined =
      resolveQueryPromisesMap.get(cacheKey);
    ...
    resolveQueryPromisesMap.set(cacheKey, promise);

    try {
      return await promise;
    } finally {
      resolveQueryPromisesMap.delete(cacheKey);
    }
